...
)
{
    ...
    if ((sd->CR1 & SPI_CR1_SPE) == 0) return RES_NOTRDY;
    //printf("this");
    disable_sdcard();
    sdcard_init_clock();
    ...
    value = sdcard_r3();
    disable_sdcard();
    int retries = 1000;  // Limit retries
do {
    enable_sdcard();
    value = sdcard_cmd(55, 0x00000000, 0x01); // CMD55 (APP_CMD)
    if (value == 0x01) {
        value = sdcard_cmd(41, 0x40000000, 0x01); // ACMD41 with HCS (SDHC/SDXC)
        //printf("ACMD41 (SDHC/SDXC) returned: 0x%02X\n", value);
